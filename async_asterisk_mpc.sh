...
# Usage: ./../async_asterisk_mpc.sh <g> <d> <players> <delay_party_count> <run_opt>
...
# Example: ./../async_asterisk_mpc.sh 10 10 5 2 0
...
if test $run_opt = 0
then
	echo "Running Asynchronous Asterisk MPC (Honest Majority)"
	echo "*****************************************************************"
    pkill -f "hm_async_asterisk_mpc"
	run_app=./benchmarks/hm_async_asterisk_mpc
	dir=~/benchmark_data/hm_async_asterisk_mpc
else
	echo "Running Asynchronous Asterisk MPC (Dishonest Majority)"
	echo "*****************************************************************"
    pkill -f "dm_async_asterisk_mpc"
	run_app=./benchmarks/dm_async_asterisk_mpc
	dir=~/benchmark_data/dm_async_asterisk_mpc
fi
