...
namespace dmGOD {
    ...
    OnlineEvaluator::OnlineEvaluator(int nP, int id, int security_param, std::shared_ptr<NetIOMP> network, PreprocCircuit<Field> preproc,
        LevelOrderedCircuit circ, int threads, uint64_t seed, int num_cheaters)
        : nP_(nP), id_(id), security_param_(security_param), num_cheaters_(num_cheaters),
        rgen_(id, nP, seed), network_(std::move(network)),
            ...
            for(size_t pid = 1; pid <= nP_; pid++) {
                tpool_->enqueue([&,pid]() {
                    ...
                    for (size_t depth = 0; depth < circ_.gates_by_level.size(); depth++) {
                        // Check if the party has been excluded before attempting to receive
                        {
                            std::lock_guard<std::mutex> lock(mtx_);
                            if (excluded_parties_.count(pid)) {
                                return;
                            }
                        }
    ...
    void OnlineEvaluator::evaluateGatesAtDepth(size_t depth) {
        ...
        if (id_ != 0) {
            bool is_cheating = id_ <= num_cheaters_;
            ...
            if(mult_num != 0) {
                ...
                if (is_cheating) {
                    mac_components[0] -= Field(1); // Introduce error
                }
                layer_dig = hashFields(mac_components);
            }

            std::vector<Field> online_comm_to_HP(total_comm_plus_dig);
            for (size_t i = 0; i < mult_num; i++) online_comm_to_HP[i] = mult_nonTP[i];
            for (size_t i = mult_num; i < total_comm_plus_dig; i++) online_comm_to_HP[i] = layer_dig[i - mult_num];
            ...
            for (size_t i=0; i<mult_num; i++) agg_values[i] = mult_nonTP[i];
            std::vector<std::future<void>> send_t;
            for(size_t pid = 1; pid <= nP_; pid++) {
            ...
            for(auto& t : send_t) if (t.valid()) t.wait();
        }

        if (id_ != 0) {
            network_->recv(0, agg_values.data(), sizeof(Field) * total_comm);
            network_->getRecvChannel(0)->flush();
            for(size_t i = 0; i < mult_num; i++) mult_all[i] = agg_values[i] + mult_nonTP[i];
        }
        else {
            bool obtained_valid_verification = false;
            ...
            while (!obtained_valid_verification)
            {
                std::unique_lock<std::mutex> lock(mtx_);
                ...
                if(message.data.size() != 0) {
                    // This party is excluded, skip Mac check
                    if (excluded_parties_.find(message.receiver_id) != excluded_parties_.end()) {
                        continue;
                    }

                    // Otherwise perform mac check
                    ...
                    fieldDig p_layer_dig(message.data.end() - 4, message.data.end());

                    if(tp_layer_dig != p_layer_dig) {
                        excluded_parties_.insert(message.receiver_id);
                    } else {
                        obtained_valid_verification = true;
                        for(size_t i = 0; i < mult_num; i++) mult_all[i] = agg_values[i] + message.data[i];
                    }
                } else {
                    obtained_valid_verification = true;
                }
    ...
    std::vector<Field> OnlineEvaluator::getOutputs() {
        ...
        if (circ_.outputs.empty()) return outvals;

        if (id_ == 0) {
            std::vector<Field> output_masks(circ_.outputs.size());
            for (size_t i = 0; i < circ_.outputs.size(); ++i) {
                output_masks[i] = preproc_.gates[circ_.outputs[i]]->mask.getValue();
            }
            std::vector<std::future<void>> send_t;
            for (size_t i = 1; i <= nP_; ++i) {
                send_t.push_back(tpool_->enqueue([&,i]() {
            ...
            network_->getRecvChannel(0)->flush();
            for (size_t i = 0; i < circ_.outputs.size(); ++i) {
                Field outmask = output_masks[i] + preproc_.gates[circ_.outputs[i]]->mask.getValue();
                outvals[i] = wires_[circ_.outputs[i]] - outmask;
            }
            return outvals;
        }
    }
