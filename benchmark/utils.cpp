...
CommPoint::CommPoint(NetIOMP& network) : stats(network.nP) {
...
StatsPoint::StatsPoint(NetIOMP& network) : cpoint_(network) {}

nlohmann::json StatsPoint::operator-(const StatsPoint& rhs) {
  return {{"time", tpoint_ - rhs.tpoint_}, {"communication", cpoint_ - rhs.cpoint_}};
}
