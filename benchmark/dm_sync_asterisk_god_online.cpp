...
void benchmark(const bpo::variables_map& opts) {
    ...
    std::shared_ptr<io::NetIOMP> network_offline = nullptr;
    std::shared_ptr<io::NetIOMP> network_online = nullptr;
    if (opts["localhost"].as<bool>()) {
        network_offline = std::make_shared<io::NetIOMP>(pid, nP+1, port, nullptr, true);
        network_online = std::make_shared<io::NetIOMP>(pid, nP+1, port, nullptr, true);
    }
    else {
        ...
        network_offline = std::make_shared<io::NetIOMP>(pid, nP+1, port, ip.data(), false);
        network_online = std::make_shared<io::NetIOMP>(pid, nP+1, port, ip.data(), false);
    }
