...
BOOST_AUTO_TEST_CASE(add) {
  ...
  std::cout << "Add Inputs " << inputs[0] << " and " << inputs[1] << std::endl;
  ...
  for (auto& p : parties) {
    if(i != 0) {
      auto output = p.get();
      std::cout << "Expected output " << exp_output[0] << " Party output " << output[0] << std::endl;
...
BOOST_AUTO_TEST_CASE(mult) {
  ...
  std::cout << "Mult Inputs " << inputs[0] << " and " << inputs[1] << std::endl;
  ...
  for (auto& p : parties) {
    if(i != 0) {
      auto output = p.get();
      std::cout << i << ": Expected output " << exp_output[0] << " Party output " << output[0] << std::endl;
      BOOST_TEST(exp_output == output);
    }
    i++;
  }
