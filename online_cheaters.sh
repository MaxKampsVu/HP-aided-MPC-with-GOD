...
then
	...
	echo "*****************************************************************"
    pkill -f "alhena_online_cheaters"
	run_app=./benchmarks/alhena_online_cheaters
    dir=~/benchmark_data/alhena_online_cheaters
else
...
do
    ...
    do
        ...
        then
            ...
            then
                $run_app -p $party --localhost -g $1 -d $2 -n $players -l $latency --num-cheaters $num_cheaters 2>&1 >> $log &
            else
                $run_app -p $party --localhost -g $1 -d $2 -n $players -l $latency $num_cheaters 2>&1 > /dev/null &
            fi
        else
            if test $party = $players
            then
                $run_app -p $party --localhost -g $1 -d $2 -n $players --num-cheaters $num_cheaters 2>&1 >> $log &
            else
                $run_app -p $party --localhost -g $1 -d $2 -n $players --num-cheaters $num_cheaters 2>&1 > /dev/null &
            fi
        fi
    ...
    $run_app -p 0 --localhost -g $1 -d $2 -n $players -t $threads 2>&1 --num-cheaters $num_cheaters | tee -a $dir/g_$1_d_$2_0.log &
    codes[0]=$!
