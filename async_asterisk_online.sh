...
if test $run_opt = 0
then
	echo "Running Asynchronous Asterisk MPC online phase (Honest Majority)"
	echo "*****************************************************************"
    pkill -f "hm_async_asterisk_online"
	run_app=./benchmarks/hm_async_asterisk_online
	dir=~/benchmark_data/hm_async_asterisk_online
else
	echo "Running Asynchronous Asterisk online phase (Dishonest Majority)"
	echo "*****************************************************************"
    pkill -f "dm_async_asterisk_online"
	run_app=./benchmarks/dm_async_asterisk_online
	dir=~/benchmark_data/dm_async_asterisk_online
fi
