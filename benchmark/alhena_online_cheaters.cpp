...
Circuit<Field> generateCircuit(size_t gates_per_level, size_t depth) {
    ...
    std::vector<wire_t> level_inputs(gates_per_level);
    std::generate(level_inputs.begin(), level_inputs.end(),
                [&]() { return circ.newInputWire(); });

    for (size_t d = 0; d < depth; ++d) {
        std::vector<wire_t> level_outputs(gates_per_level);

        for (size_t i = 0; i < gates_per_level - 1; ++i) {
            level_outputs[i] = circ.addGate(GateType::kMul, level_inputs[i], level_inputs[i + 1]);
        }
        level_outputs[gates_per_level - 1] =
            circ.addGate(GateType::kMul, level_inputs[gates_per_level - 1], level_inputs[0]);
...
void benchmark(const bpo::variables_map& opts) {
    ...
    auto num_cheaters = opts["num-cheaters"].as<int>();

    // CHECK: Depth must be greater than number of cheaters

    if (num_cheaters >= nP) {
        throw std::runtime_error("num_cheaters can't be equal/greater than num_parties");
    }
    ...
    if (opts["localhost"].as<bool>()) {
        ...
        std::vector<char*> ip(nP+1);
        for (size_t i = 0; i < nP+1; ++i) {
            ipaddress[i] = netdata[i].get<std::string>();
            ip[i] = const_cast<char*>(ipaddress[i].data());
        }
    ...
    // Pass num_cheaters to the OnlineEvaluator
    OnlineEvaluator eval(nP, pid, security_param, network, std::move(preproc), circ, threads, seed, num_cheaters);
...
bpo::options_description programOptions() {
    ...
    desc.add_options()
        ...
        ("threads,t", bpo::value<size_t>()->default_value(64), "Number of threads.")
        ("latency,l", bpo::value<size_t>()->default_value(0), "Latency in ms.")
        ...
        ("num-cheaters", bpo::value<int>()->default_value(0), "Number of cheaters to simulate.");
